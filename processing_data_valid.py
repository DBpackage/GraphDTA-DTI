...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_valid)) or (not os.path.isfile(processed_data_file_test))):
        ...
        valid_drugs, valid_prots, valid_Y, valid_Y_c = list(df['smiles']),list(df['sequence']),list(df['pka']), list(df['label'])
        XT = [seq_cat(t) for t in valid_prots]
        valid_drugs, valid_prots, valid_Y, valid_Y_c = np.asarray(valid_drugs), np.asarray(XT), np.asarray(valid_Y), np.asarray(valid_Y_c)
        ...
        valid_data = TestbedDataset(root='data', dataset=dataset+'_valid', xd=valid_drugs, xt=valid_prots, y=valid_Y, y_c=valid_Y_c, smile_graph=smile_graph)
        ...
        print(processed_data_file_train, ',', processed_data_file_valid,'and',processed_data_file_test, 'are already created')
